...
/// Lifecycle events agents report through hooks (`foreman://agent-event?e=<kind>&pid=<pid>`).
...
enum AgentEventURL {
    static let scheme = "foreman"
    static let host = "agent-event"

    /// Any local process can open a `foreman://` URL, so only a known kind and a plausible pid pass;
    ...
    static var codexNotifySnippet: String {
        "notify = " + encode(["/bin/sh", "-c", hookCommand(.stop), "foreman"], pretty: false)
    }
