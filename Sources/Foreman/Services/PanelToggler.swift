...
/// that, so this clicks Foreman's own status item button, which behaves exactly like a user click.
@MainActor
enum PanelToggler {
    @discardableResult
    static func toggle() -> Bool {
        guard let button = statusButton() else {
            ...
            NSLog("Foreman: status item button not found, cannot toggle the panel")
            return false
        }
