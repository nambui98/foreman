...
struct AgentEventURLTests {
    @Test func parsesValidEvents() throws {
        let parsed = try #require(AgentEventURL.parse(URL(string: "foreman://agent-event?e=stop&pid=4242")!))
        #expect(parsed.kind == .stop)
        #expect(parsed.pid == 4242)
        #expect(AgentEventURL.parse(URL(string: "foreman://agent-event?pid=7&e=start")!)?.kind == .start)
    }

    @Test func rejectsMalformedEvents() {
        let bad = [
            "http://agent-event?e=stop&pid=42",      // wrong scheme
            "foreman://other?e=stop&pid=42",         // wrong host
            "foreman://agent-event?e=rm&pid=42",     // unknown kind
            "foreman://agent-event?e=stop&pid=1",    // launchd
            "foreman://agent-event?e=stop&pid=-5",
            "foreman://agent-event?e=stop&pid=abc",
            "foreman://agent-event?e=stop",
        ]
...
struct AgentEventCenterTests {
    func makeCenter() throws -> (AgentEventCenter, AppSettings, Box) {
        let settings = AppSettings(defaults: try #require(UserDefaults(suiteName: "foreman-events-\(UUID())")))
