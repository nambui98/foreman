...
The interface is in Vietnamese. Personal-use tool: non-sandboxed, ad-hoc signed, not on the App Store.
...
Optional: Settings (⌘, or the ⚙︎ in the panel footer) → **Chung** → *Mở Foreman khi đăng nhập* to start at login.

## Using Foreman

### Cổng (Ports) tab
Processes are grouped into **Dev** (node, bun, python, …), **Database / Container** (postgres, redis, OrbStack, …) and **Hệ thống** (other apps and daemons, collapsed).
...
| Open folder in your editor | Right-click → *Mở trong Cursor* (editor chosen in Settings) |
...
- **mồ côi** (orphan) — the parent exited (terminal or agent closed), or the working folder was deleted (e.g. a removed git worktree).
- **rảnh 5h** (idle) — no inbound connection and no CPU, older than the idle threshold (2 h by default), confirmed over 3 refreshes.

When any exist, **Dọn (N)** appears in the header: tick what to stop (orphans pre-ticked, idle ones not) and confirm.

### Agents tab
| Status | Meaning |
|---|---|
| 🟡 **Chờ bạn** | Waiting for you (permission prompt or question) — listed first |
| 🟢 **Đang chạy** | Working, including while waiting on the model |
| ⚪ **Rảnh** | Turn finished, waiting for your next prompt |
| 🟠 **Tạm dừng** | Its child processes are paused by Foreman |

Hover the status dot to see where it came from: *theo Orca* (exact, from Orca's agent hooks) or *ước lượng theo CPU* (estimated from process-tree CPU, for agents outside Orca).
...
- **Other agents** — add Foreman's hooks for exact, instant events. Settings → **Agents** has a **Copy** button for each:
  ...
  **Codex** — add to `~/.codex/config.toml`:
  ...
  ```
  Codex only reports finished turns, so every Codex turn notifies. Foreman never edits these files itself. *Sự kiện gần nhất* in Settings shows the last event received, to check your setup.
...
| Cổng | Editor for *Mở trong …* | first installed of Cursor, VS Code, Zed, Sublime Text, Xcode |
...
- Stop anything from the *Hệ thống* section, a process group, an agent tree or a cleanup batch without asking first. Before each cleanup signal it re-checks the process start time, so a recycled PID is never hit.
...
| ⌥⌘P does nothing / Settings says the shortcut is taken | Another app uses it — record a different combination in Settings → Chung |
| ↗ only brings Terminal/iTerm to the front | Allow Foreman under System Settings → Privacy & Security → Automation |
| No notifications | Allow Foreman under System Settings → Notifications; for agents outside Orca, install the hooks and check *Sự kiện gần nhất* |
| An agent shows *Rảnh* while it works | Outside Orca, status is estimated from CPU (the tooltip says so) — install the hooks for exact status |
